...
declare let L: any; 
@Component({
  selector: 'app-m-product-display',
  templateUrl: './m-product-display.component.html',
  styleUrls: ['./m-product-display.component.css']
})
export class MProductDisplayComponent {
  productDetails:any;
  fieldDetails: any;
  map: any;
  constructor(private route: ActivatedRoute, private fieldService: FieldDetailsService,private router: Router,private apiKeyService: ApiKeyService) {}

  ngOnInit(): void {
    const fieldId = this.route.snapshot.params['id'];
    this.fieldService.getProductByfieldId(fieldId).subscribe( product=>{
